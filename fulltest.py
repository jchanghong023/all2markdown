...
默认调用 init.cmd 安装的 GitHub Latest Release 运行时 xberg.exe
（%LOCALAPPDATA%\\all2markdown\\xberg\\latest\\runtime，可用 --cli/--pkg-dir 覆盖），
输出到 D:\\测试转markdown转换效果\\测试文档_md_fulltest。
...
def _default_runtime() -> Path:
    """init.cmd 安装的 Xberg 运行时目录；路径契约与 src/runtime_paths.py 一致。"""
    override = os.environ.get("ALL2MARKDOWN_DATA_DIR", "").strip()
    if override:
        return Path(override).expanduser() / "xberg" / "latest" / "runtime"
    local = os.environ.get("LOCALAPPDATA", "").strip()
    base = Path(local) if local else Path.home() / "AppData" / "Local"
    return base / "all2markdown" / "xberg" / "latest" / "runtime"


DEFAULT_RUNTIME = _default_runtime()
DEFAULT_CLI = DEFAULT_RUNTIME / "xberg.exe"
# 运行时目录提供包内模型缓存（HF_HUB_CACHE=runtime/models）和 DLL 搜索路径
PKG_DIR = DEFAULT_RUNTIME
...
def main():
    ...
    if pkg_dir.is_dir():
        ...
        # 与 src/all2markdown_core.py 的转换环境一致：转换阶段强制离线、固定 CPU EP
        env["HF_HUB_OFFLINE"] = "1"
        env["HUGGINGFACE_HUB_OFFLINE"] = "1"
        env["TRANSFORMERS_OFFLINE"] = "1"
        env["HF_DATASETS_OFFLINE"] = "1"
        env["XBERG_ORT_EP"] = "cpu"
        ort_dll = pkg_dir / "onnxruntime.dll"
        if ort_dll.is_file():
            env["ORT_DYLIB_PATH"] = str(ort_dll)
